...
def lst_to_matrix(lst):
    ''' Convert list of elements of n^2 length -> n*n matrix 
        Equivalent can be achieved via Numpy library rehsape function
         \
          np.reshape(lst, (d,d))
    '''
    s = len(lst)
    d = isqrt(s)  # assuming d*d == s
...
def generateMatrix(n: int) -> List[List[int]]:
    ...
    # it.chain(*[it.repeat(j) for j in range(n-1, 1,-1)])
